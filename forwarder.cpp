...
if(!interest.getName()[0].toUri().compare("ndn"))
{
uint64_t my_nextSegmentNo =  interest.getName()[-1].toSegment();
std::cerr << " before " << interest.getName().toUri() << "  " << my_nextSegmentNo << std::endl;
//my_nextSegmentNo = my_nextSegmentNo +11 ;
if (interest.getName()[-1].toSegment() == 0)
{
 // std::string newInterestName = data.getName()[0].toUri() + "/" + data.getName()[1].toUri() +"/" +data.getName()[2].toUri();
  //Name newPrefix(newInterestName);
  //Interest newInterest(Name(newPrefix).appendSegment(my_nextSegmentNo));
//                std::cout << "Next data: " << newInterest.getName() << std::endl;

    //            std::string newName = newInterest.getName().toUri();
              //  std::cerr << "starting data from nfd.. " << newName << std::endl;

 try{ 
      //Erase previous message queue
      boost::interprocess::message_queue::remove("message_queue");
      
      std::string s = "Mohd";//newName;
      //Create a message_queue.
      boost::interprocess::message_queue mq (boost::interprocess::create_only,"message_queue",100,s.size());
      
      //size_t const max_msg_size = 0x100;
      
      //std::cout << "Sending s : " << s.data() << std::endl;
